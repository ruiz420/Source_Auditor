...
STATE_OVERRIDES_FILE = os.path.join(app.root_path, "estado_fuentes.json")
...
def get_user_key():
    return session.get("username") or session.get("session_id") or "default"


def load_state_overrides():
    if not os.path.exists(STATE_OVERRIDES_FILE):
        return {}

    try:
        with open(STATE_OVERRIDES_FILE, "r", encoding="utf-8") as file:
            return json.load(file)
    except (json.JSONDecodeError, OSError):
        return {}


def save_state_overrides(data):
    with open(STATE_OVERRIDES_FILE, "w", encoding="utf-8") as file:
        json.dump(data, file, ensure_ascii=False, indent=2)


def get_saved_url_state(url):
    overrides = load_state_overrides()
    return overrides.get(get_user_key(), {}).get(url)


def save_url_state(url, state_value):
    overrides = load_state_overrides()
    user_key = get_user_key()
    overrides.setdefault(user_key, {})[url] = state_value
    save_state_overrides(overrides)


def normalize_state(value):
    clean_value = str(value or "").strip().lower()

    if clean_value in ("valid", "valida", "válida", "ok", "valido", "válido"):
        return "valid"

    if clean_value in ("error", "invalid", "invalida", "inválida", "invalido", "inválido"):
        return "error"

    return "valid" if clean_value == "valida" else "error"


def state_from_status(status):
    return "valid" if str(status).upper() == "VALIDA" else "error"


def display_status_from_state(state_value):
    return "VALIDA" if state_value == "valid" else "ERROR"
...
def audit_item(item):
    ...
    data["state"] = get_saved_url_state(original_url) or state_from_status(data["status"])
    data["manual"] = data["state"]
...
def result_payload(url, data):
    return {
        "url": url,
        "request_url": data.get("request_url", ""),
        "final_url": data.get("final", ""),
        "redirected": bool(data.get("redirected", False)),
        "status": display_status_from_state(data.get("state", state_from_status(data.get("status", "")))),
        ...
        "agent_label": data.get("agent_label", ""),
        "screenshot": data.get("screenshot", ""),
        "state": data.get("state", state_from_status(data.get("status", ""))),
    }
...
def current_state():
    ...
    for item in state["urls"]:
        ...
        if original_url in results:
            ordered_results.append(result_payload(original_url, results[original_url]))
...
def mark():
    ...
    url = data["url"]
    state_value = normalize_state(data.get("state") or data.get("status"))

    if url not in results:
        results[url] = {}

    results[url]["manual"] = state_value
    results[url]["state"] = state_value
    results[url]["status"] = display_status_from_state(state_value)
    save_url_state(url, state_value)

    return jsonify({"ok": True, "state": state_value, "status": display_status_from_state(state_value)})
...
def export():
    ...
    for url, data in results.items():
        state_value = data.get("state", state_from_status(data.get("status", "")))
        writer.writerow([
            url,
            display_status_from_state(state_value),
            data.get("final", ""),
            data.get("reason", ""),
            data.get("http_code", ""),
            data.get("title", ""),
...
def export_excel():
    ...
    for r_idx, (url, data) in enumerate(results.items(), start=2):
        state_value = data.get("state", state_from_status(data.get("status", "")))
        status = display_status_from_state(state_value)
        final_url = data.get("final", "")
        reason = data.get("reason", "")
        http_code = data.get("http_code", "")
        title = data.get("title", "")
